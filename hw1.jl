...
function vecvec_to_matrix(vecvec)
	row = size(vecvec, 1)
	col = size(vecvec[1], 1)
	result = Matrix{Int64}(undef, row, col)
	for i in 1:row
		for j in 1:col
...
function matrix_to_vecvec(matrix)
	vec_size, elem_size = size(matrix)
    vec = [Vector{Int64}(undef,elem_size) for _ in 1:vec_size]
	for i in 1:vec_size
		vec[i] = matrix[i,:]
	end
	return vec
